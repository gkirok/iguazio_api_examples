...
//                                git checkout ${V3IO_TSDB_VERSION}
                    ...
                    stage('build in dood') {
                        container('docker-cmd') {
                            sh """
                                cd ${BUILD_FOLDER}/src/github.com/v3io/iguazio_api_examples/netops_demo/golang/src/github.com/v3io/demos
                                docker build . --tag netops-demo-golang:\\${NETOPS_TAG} --tag ${docker_user}/netops-demo-golang:$NETOPS_DEMO_VERSION --build-arg NUCLIO_BUILD_OFFLINE=true --build-arg NUCLIO_BUILD_IMAGE_HANDLER_DIR=github.com/v3io/demos

                                cd ${BUILD_FOLDER}/src/github.com/v3io/iguazio_api_examples/netops_demo/py
                                docker build . --tag netops-demo-py:\\${NETOPS_TAG} --tag ${docker_user}/netops-demo-py:$NETOPS_DEMO_VERSION
                            """
